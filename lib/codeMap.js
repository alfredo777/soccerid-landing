...
function matchOwner(code, visitor) {
  ...
  if (oMail && vMail) return oMail === vMail;
  if (oPhone && vPhone) return oPhone === vPhone;
  // Sin correo ni teléfono comparables, el nombre es lo único que queda.
  // Es débil (hay homónimos), pero es mejor que decir "otra persona" sin base.
  if (oName && vName) return oName === vName;
  return null;
}
