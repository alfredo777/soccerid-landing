...
async function notify({ type, title, body, audience, userId, eventId, channels }) {
  ...
  for (const u of recipients) {
    if (chans.includes('email') && u.email && wantsEmail(u)) {
    ...
    if (chans.includes('sms') && u.phone && wantsSms(u)) {
...
module.exports = { notify, notifyAdmins, TYPES, TYPE_KEYS, normChannels, normType };
