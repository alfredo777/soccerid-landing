...
<table width="100%" style="background:#f4f2f8;padding:32px 0;"><tr><td align="center">
<table width="600" style="background:#fff;border-radius:16px;overflow:hidden;border:1px solid #ecebf2;">
  <tr><td style="background:#6C3CE0;padding:26px 40px;text-align:center;"><img src="https://soccerid.co/assets/images/soccerid.png" alt="SOCCER iD" height="30"></td></tr>
  <tr><td style="padding:38px 40px;">
    ${name ? `<p style="color:#14141b;font-size:16px;font-weight:700;margin:0 0 14px;">Hola ${name},</p>` : ''}
    <div style="color:#55505f;font-size:15px;line-height:1.65;white-space:pre-line;">${(body || '').replace(/</g, '&lt;')}</div>
  </td></tr>
  <tr><td style="background:#14141b;padding:18px 40px;text-align:center;"><span style="color:rgba(255,255,255,0.4);font-size:11px;">&copy; ${new Date().getFullYear()} SOCCER iD</span></td></tr>
</table></td></tr></table></body></html>`;
  try {
    await tx.sendMail({ from: process.env.SMTP_FROM || '"SOCCER iD" <socceridco@soccerid.co>', to, subject: subject || 'SOCCER iD CUP 2027', html, text: body || '' });
    return { sent: true };
  } catch (err) {
    return { sent: false, error: err.message };
  }
