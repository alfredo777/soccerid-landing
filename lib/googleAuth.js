 ...
 *
 * `GOOGLE_LOGIN=0` oculta el botón y cierra las rutas sin borrar las credenciales
 * (útil mientras la pantalla de consentimiento sigue en Testing). Ver docs/google-auth.md.
 *
...
// Interruptor explicito para ocultar el login de Google sin perder las
// credenciales: GOOGLE_LOGIN=0|off|false|no lo apaga aunque esten configuradas.
// Sin la variable, manda tener client id + secret (comportamiento de siempre).
function switchedOff() {
  const v = String(process.env.GOOGLE_LOGIN == null ? '' : process.env.GOOGLE_LOGIN).trim().toLowerCase();
  return v === '0' || v === 'off' || v === 'false' || v === 'no';
}

// Apagado aqui cubre todo: el boton del login y las dos rutas de OAuth.
function enabled() { if (switchedOff()) return false; const c = config(); return !!(c.id && c.secret); }
