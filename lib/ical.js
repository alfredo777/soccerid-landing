...
/**
 * Saca un rango de fechas de la ETIQUETA de una etapa ("Oct 2026",
 * "27 Mar 2027", "Abr - Jul 2027").
 *
 * No es adivinar: la etiqueta es la fecha que el organizador ya escribió, solo
 * que como texto libre. Lo que se hace aquí es leerla. Un mes suelto se toma
 * completo (del 1 al último día), que es lo que significa "esta etapa ocurre en
 * octubre"; un día explícito se respeta tal cual.
 *
 * Devuelve null si no se entiende, y entonces no se ofrece agendar: es preferible
 * no dar el botón a mandar a alguien a una fecha equivocada.
 */
const MESES_ES = {
  ene: 1, enero: 1, feb: 2, febrero: 2, mar: 3, marzo: 3, abr: 4, abril: 4,
  may: 5, mayo: 5, jun: 6, junio: 6, jul: 7, julio: 7, ago: 8, agosto: 8,
  sep: 9, set: 9, septiembre: 9, sept: 9, oct: 10, octubre: 10,
  nov: 11, noviembre: 11, dic: 12, diciembre: 12
};

function finDeMes(anio, mes) { return new Date(Date.UTC(anio, mes, 0)).getUTCDate(); }

function fechasDeEtiqueta(label) {
  const txt = String(label || '')
    .toLowerCase()
    .normalize('NFD').replace(/[̀-ͯ]/g, '')   // quita acentos
    .replace(/[‐-―]/g, '-')                    // guiones "bonitos" a '-'
    .trim();
  if (!txt) return null;

  const anioM = txt.match(/(20\d{2})/g);
  if (!anioM) return null;
  const anioFin = parseInt(anioM[anioM.length - 1], 10);
  const anioIni = parseInt(anioM[0], 10);

  // Día explícito: "27 mar 2027"
  const dia = txt.match(/(^|[^0-9])([0-9]{1,2})[^0-9a-z]+([a-z]+)/);
  const meses = [];
  const re = /([a-z]+)/g;
  let m;
  ...
  if (dia && MESES_ES[dia[3]] && meses.length === 1) {
    const d = parseInt(dia[2], 10);
    if (d >= 1 && d <= finDeMes(anioFin, meses[0])) {
      const iso = `${anioFin}-${dosDig(meses[0])}-${dosDig(d)}`;
      return { desde: iso, hasta: iso, aproximado: false };
    }
  }

  const mesIni = meses[0], mesFin = meses[meses.length - 1];
  // "Dic 2026 - Feb 2027": si el mes final es menor y hay dos años, el segundo manda
  const yIni = (meses.length > 1 && anioM.length > 1) ? anioIni : (mesFin < mesIni ? anioFin - 1 : anioFin);
  return {
    desde: `${yIni}-${dosDig(mesIni)}-01`,
    hasta: `${anioFin}-${dosDig(mesFin)}-${dosDig(finDeMes(anioFin, mesFin))}`,
    aproximado: true
  };
}

/**
 * Lo mismo, pero para UNA etapa del cronograma.
 * Una etapa es un tramo, no un momento: va como evento de día completo del
 * inicio al fin. `DTEND` de Google es exclusivo, así que se suma un día al
 * final; si no, una etapa que termina el 30 se vería terminando el 29.
 * Devuelve '' si la etapa no tiene fecha: sin fecha no hay nada que agendar.
 */
function rangoDeEtapa(e) {
  const iso = /^\d{4}-\d{2}-\d{2}$/;
  // Las fechas capturadas SIEMPRE ganan: si el organizador las puso, son las
  // buenas. La etiqueta es el respaldo para lo que nunca se capturó.
  const desde = String(e.startDate || e.start_date || e.endDate || e.end_date || '');
  if (iso.test(desde)) {
    const hastaRaw = String(e.endDate || e.end_date || '');
    return { desde, hasta: iso.test(hastaRaw) ? hastaRaw : desde, aproximado: false };
  }
  return fechasDeEtiqueta(e.date || e.date_label || '');
}

function enlaceGoogleEtapa(e, base = 'https://soccerid.co') {
  const r = rangoDeEtapa(e);
  if (!r) return '';
  const desde = r.desde, hasta = r.hasta;
  const d = new Date(hasta + 'T00:00:00Z');
  d.setUTCDate(d.getUTCDate() + 1);
  const p = new URLSearchParams({
    action: 'TEMPLATE',
    text: e.title,
    dates: `${desde.replace(/-/g, '')}/${d.toISOString().slice(0, 10).replace(/-/g, '')}`,
    details: [e.description, e.owner, `${base}/panel/cronograma`].filter(Boolean).join('\n'),
    ctz: 'America/Mexico_City'
  });
  return `https://calendar.google.com/calendar/render?${p.toString()}`;
}

module.exports = { tokenPara, usuarioDe, construir, enlaceGoogle, enlaceGoogleEtapa, rangoDeEtapa, fechasDeEtiqueta };
