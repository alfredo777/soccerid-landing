...
/**
 * Sigue redirecciones A MANO, revalidando CADA salto contra `urlSegura`. Con
 * `redirect: 'follow'` una URL pública que responde 302 hacia 169.254.169.254
 * (metadatos de la nube) o localhost se seguiría a ciegas (SSRF). Aquí cada
 * `Location` se vuelve a validar (IP/host) antes de seguirlo.
 */
async function fetchSeguro(u, opts, maxHops = 4) {
  let actual = u;
  for (let i = 0; i < maxHops; i++) {
    const res = await fetch(actual.href, Object.assign({ redirect: 'manual' }, opts));
    if (res.status >= 300 && res.status < 400 && res.headers.get('location')) {
      const dest = new URL(res.headers.get('location'), actual.href);
      actual = await urlSegura(dest.href);   // revalida antes de seguir
      continue;
    }
    return res;
  }
...
/**
 * Extrae el TEXTO legible del artículo (para que la IA redacte a partir de él,
 * no solo del og:description). Prefiere el <article>; saca los párrafos/títulos
 * de lista, ya decodificados y sin tags.
 */
function textoArticulo(html) {
  let zona = html;
  const art = html.match(/<article[\s\S]*?<\/article>/i);
  if (art) zona = art[0];
  zona = zona.replace(/<(script|style|noscript|figure|nav|footer|header|aside)[\s\S]*?<\/\1>/gi, ' ');
  const parrafos = [];
  const re = /<(p|li|h2|h3|blockquote)[^>]*>([\s\S]*?)<\/\1>/gi;
  let m;
  while ((m = re.exec(zona)) && parrafos.length < 150) {
    const txt = decodificar(m[2].replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ')).trim();
    if (txt.length > 25) parrafos.push(txt);
  }
  return parrafos.join('\n\n').slice(0, 9000);
}

/**
 * @returns {{url, titulo, descripcion, imagen, imagenes:string[], fuente, texto}}
 */
