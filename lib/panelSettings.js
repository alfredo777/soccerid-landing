...
function fileDefaults() {
  ...
  return {
    ...
    // Etapa actual del proyecto (banda de "Estado del evento" en el dashboard)
    stageLabel: 'Comercialización',
    stageStep: 3,
    stageTotal: 6,
    stageNote: 'Venta de patrocinios y preventa de boletos en curso.',
    stageUpdated: '',
    // Sello de trazabilidad para las cifras de inversión
    returnSource: 'Modelo financiero SOCCER iD',
    returnUpdated: '',
    // Edición activa: la elige el ADMIN y es la que ven todos los inversionistas.
    // Vacío = se toma la edición más próxima (mayor año). El inversionista NO la cambia.
    activeEditionId: '',
    // Ventas y punto de equilibrio
    ticketsSold: 0,
    salesUpdated: '',
    breakEvenTickets: '' // vacío = se calcula como costo del proyecto / precio de boleto
  };
}
...
function computeReturn(user, cfg) {
  ...
  // Override de rendimiento por cuenta (si el admin lo definió para este usuario)
  const hasOverride = user.return_rate !== null && user.return_rate !== undefined && user.return_rate !== '' && !isNaN(Number(user.return_rate));
  const applyOverride = (base) => {
    if (!hasOverride) return base;
    const rate = Number(user.return_rate);
