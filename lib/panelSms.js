 ...
 *
 * **Las llaves son el interruptor**: en cuanto están las tres (SID, token y
 * remitente), el canal SMS queda activo. No hay un switch aparte, para que no se
 * quede apagado por olvido después de pegar las credenciales.
 *
...
async function getConfig() {
  ...
  // Las llaves SON el interruptor: con las tres puestas, el SMS queda activo.
  // No hay un switch aparte que se pueda quedar apagado por olvido.
  cfg.ready = !!(cfg.account_sid && cfg.auth_token && cfg.from);
  cfg.configured = cfg.ready;
  // Qué falta, para poder decirlo en el admin en vez de un "no funciona" a secas
  cfg.missing = ['account_sid', 'auth_token', 'from'].filter(k => !cfg[k]);
  return cfg;
}
...
async function getPublicConfig() {
  ...
  const FALTA = { account_sid: 'el Account SID', auth_token: 'el Auth Token', from: 'el remitente' };
  return {
    account_sid: c.account_sid,
    from: c.from,
    ready: c.ready,
    partial: !c.ready && c.missing.length < 3,
    missingLabel: c.missing.map(k => FALTA[k]).join(' y '),
...
async function sendSms({ to, body }) {
  ...
  if (!cfg.ready) {
    console.log(`  ⚠ Twilio sin llaves — SMS a ${dest} no enviado`);
    return { sent: false, error: 'Faltan las llaves de Twilio' };
  }
